      ...
      if ($user) {
        ...
        if ($user.object.displayName && $user.object.email) {
          interviewer.interviewerFirstName =
            $user.object.displayName.split(' ')[0]
          interviewer.interviewerLastName =
            $user.object.displayName.split(' ')[1]
  ...
  >
    ...
    {#each value as interview}
      {#if editSlots}
        ...
        {/if}
      {:else if (!includeAllSlots && interview.interviewerEmail === interviewer.interviewerEmail) || includeAllSlots}
        <Card>
          <div class="my-1">
            <b>Interviewer:</b>
